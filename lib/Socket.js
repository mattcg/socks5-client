...
function handleSocksConnectToHost(client) {
	client.socket.on('close', function(hadErr) {
		client.emit('close', hadErr);
	});
	...
	client.socket._httpMessage = client._httpMessage;
	client.socket.parser = client.parser;
	client.socket.ondata = client.ondata;
